import type { InterviewCommentModel, NewInterviewCommentModel } from '@job-seekr/data/validation'
...
type FormState =
  Omit<NewInterviewCommentModel, 'comment_date'> & { comment_date: string };
  ...
  return (
    <form className="m-8 p-8 rounded-xl shadow-xl" onSubmit={(e) => {
      ...
      <div>
        ...
        <div>
          <label className='text-sm font-bold text-gray-700' htmlFor="pinned">Pinned</label>
          <input type="checkbox" id="pinned" name='pinned' checked={form.pinned} className='ml-2' onChange={e => setForm(oldForm => ({ ...oldForm, pinned: e.target.checked }))} />
        </div>
        ...
        <div className="form-input">
          ...
          <textarea
            ...
            name="comment"
            value={form.comment}
            onChange={e => setForm(oldForm => ({ ...oldForm, comment: e.target.value }))}></textarea>
        </div>
      </div>
      <div className="form-actions">
        ...
        <button disabled={props.isBusy} onClick={() => props.onCancel()} type='button' className="btn gray">Cancel</button>
      </div>
    </form>
  )
}
